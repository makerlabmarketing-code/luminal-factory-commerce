...
- **Operator configuration report:** The owner reported Supabase custom SMTP and Turnstile protection configured successfully and added the public Turnstile site key to Vercel Preview Config. These settings are not treated as end-to-end verified until an exact-head Preview smoke passes; customer Auth runtime remains false.
- **Current gate:** `SLICE_B_AUTH_UI_CODE_COMPLETE_STAGING_APPROVAL_REQUIRED`. The exact-head Auth staging runbook is prepared, but no OTP has been requested and no Auth user/session has been created by this slice.
- **Next approval:** Create one exact-head Preview for the accumulated branch, run the disabled probe, then explicitly approve the bounded one-email/one-user enabled OTP smoke and restore Preview false. Keep Production guest cart/Auth false or absent; customer RLS, cart attachment, saved addresses and global Account navigation remain disconnected.
