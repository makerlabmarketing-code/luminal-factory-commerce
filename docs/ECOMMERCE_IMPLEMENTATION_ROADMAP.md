...
- **Current gate:** `SLICE_B_CUSTOMER_CART_MERGE_ADAPTER_REVIEW_REQUIRED`. Add the server-only Supabase adapter from the generated signature and keep it disconnected/default-off until a separately approved runtime integration smoke. Customer browser RLS, saved addresses, order history and global Account navigation remain deferred.
