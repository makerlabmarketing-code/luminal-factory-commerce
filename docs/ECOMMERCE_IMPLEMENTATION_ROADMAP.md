...
- **Mobile/reduced motion:** Mobile uses a reduced-distance version of the same Hero → Meet scroll choreography so the 3D continuity is preserved without copying desktop geometry 1:1. The mobile Hero reserves a lower visual stage for the GLB so the object does not cover title, story, or CTA copy; the model then travels upward into Meet Meowhe as scrolling begins. Coarse pointer does not block page scroll or add hover interaction. Reduced motion still skips the scroll choreography.
