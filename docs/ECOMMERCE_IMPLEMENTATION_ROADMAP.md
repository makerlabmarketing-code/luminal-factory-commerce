...
- **Current gate:** `SLICE_B_CUSTOMER_CART_MERGE_RPC_MIGRATION_REVIEW_REQUIRED`. `specs/commerce/phase6-customer-cart-merge-technical-plan.md` owns the atomic invoker-RPC, lock ordering, idempotent private receipt and retention design. Customer RLS, transactional cart attachment runtime, saved addresses and global Account navigation remain disconnected. Production SQL and runtime activation require separate reviewed gates.
