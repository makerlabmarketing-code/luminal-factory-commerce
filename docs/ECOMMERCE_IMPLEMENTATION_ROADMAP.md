...
- **HOME-3D-PATH-02:** `OWNER_APPROVED_FOR_PRODUCTION_2026-09-23`. The traveling object is intentionally bounded to **Hero → Meet Meowhe**. The Hero no longer carries the extra object-description note; the 3D itself owns the visual field. On desktop the model follows pointer position without click-and-drag, then shifts left and turns into a stronger **left-facing** camera orbit at Meet Meowhe before fading out. Mobile keeps the non-travel layout but now shows an inline touch-safe 3D Meowhe in the Meet section, with constrained-network fallback preserved.
