...
- **Operator configuration report:** The owner reported Supabase custom SMTP and Turnstile protection configured successfully and added the public Turnstile site key in Vercel. Exact Production scope/value evidence must still pass the runbook preflight; customer Auth remains false.
- **Current gate:** `SLICE_B_AUTH_UI_DEPLOYED_PRODUCTION_SMOKE_APPROVAL_REQUIRED`. `specs/commerce/phase6-customer-auth-production-smoke-runbook.md` is prepared for exactly one login using `tungduy165@gmail.com`; no OTP has been requested by this slice.
- **Next approval:** After Production environment preflight, explicitly approve the bounded enable → one OTP → verify → refresh → sign-out → disable sequence. Customer RLS, cart attachment, saved addresses and global Account navigation remain disconnected.
