...
- **Customer/cart merge domain and adapter:** `src/features/cart/customer-cart-merge.ts` defines the default-off verified-subject boundary. The local server-only Supabase adapter now calls only the generated `merge_verified_customer_guest_cart` signature, validates its single-row bounded result and maps every database/configuration failure closed. Privileged client construction requires Auth, guest cart and merge flags all to be exact `true`; no route or Auth consumer imports it.
...
- **Current gate:** `SLICE_B_CUSTOMER_CART_MERGE_RUNTIME_INTEGRATION_REVIEW_REQUIRED`. Review the post-verification caller and guest-cookie success/failure contract before connecting the adapter behind the three default-false flags. Production activation and smoke remain separate live gates. Customer browser RLS, saved addresses, order history and global Account navigation remain deferred.
