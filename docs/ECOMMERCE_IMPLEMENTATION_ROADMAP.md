...
- **Delivery evidence:** PR #39 merged the complete disabled guest-cart foundation and staging gate as `e0f13ca43e19783439abaca845ee8baa4cc01bf6`; GitHub CI and Vercel Preview passed.
- **Current gate:** `STAGING_DISABLED_PROBE_BLOCKED_AUTOMATION_BYPASS_REQUIRED`. The Preview is protected by Vercel Authentication, so unauthenticated automation receives Vercel's `401` envelope before `/api/cart`. No application request or database write occurred.
- **Next approved operation:** Configure a Vercel Protection Bypass for Automation secret only in the operator environment and rerun the non-mutating disabled probe. Enabled Preview runtime, redeployment and the bounded create/delete smoke remain a separate explicit approval. Production runtime remains false/absent.
