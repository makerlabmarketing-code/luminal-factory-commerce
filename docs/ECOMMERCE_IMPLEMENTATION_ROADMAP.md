...
- **Latest staging evidence:** Exact-head Preview `8d030115ddf24991d2086e026f7eaf35f4b5e7de` reached `READY`. The approved enabled smoke could not issue its POST from the current outbound-restricted execution environment, and the storefront intentionally had no Cart UI or published product to use as a substitute. No cart was created. Preview runtime was restored to false and redeployed as `dpl_6syWnHQLescZjUQpMPtNFPurgeTF`; postflight remained zero carts, cart items, Auth users and limiter rows, with one active cleanup job. Production and Auth were untouched.
- **Current gate:** `ENABLED_SMOKE_RUNNER_REQUIRED`. A manual-only, read-only-permission GitHub Actions runner is prepared to execute the existing verifier with normal HTTPS egress. It requires GitHub Actions secrets, explicit enabled-write confirmation and the same exact-head isolated Preview proof. Cart UI and Auth remain disconnected until this backend smoke passes.
- **Next approved operation:** Validate and merge the staging runner once, configure its two GitHub Actions secrets, run disabled mode on a proven exact-head Preview, then use the already approved Preview-only enabled/redeploy/create-delete/postflight/false sequence. Keep Production runtime false/absent.
