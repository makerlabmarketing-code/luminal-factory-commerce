...
- **HOME-3D-PATH-02:** `IN_PROGRESS`. The traveling object is now intentionally bounded to **Hero → Meet Meowhe**. At the Meet Meowhe stop it shifts further left, scales down, and rotates to a right-facing camera orbit to contrast with the front-facing Hero. It then fades out before the next studio/revival chapter instead of continuing through Archive/Gallery.
