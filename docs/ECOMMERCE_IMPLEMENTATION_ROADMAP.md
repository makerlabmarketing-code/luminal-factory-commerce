...
- **Current gate:** `SLICE_B_AUTH_STAGING_CONFIGURATION_REQUIRED`. The default-off OTP boundary and durable abuse limiter are code/database complete. No Account UI, Supabase Auth/SMTP/Turnstile configuration, customer RLS grant, cart attachment, PII or address storage is enabled.
- **Next approval:** Review an isolated Auth staging configuration/runbook for Cloudflare Turnstile and custom SMTP, then provision Preview-only values and run disabled-before-enabled OTP smoke. Keep `COMMERCE_CUSTOMER_AUTH_ENABLED=false`, Production guest cart false/absent and Production Auth unchanged until that separate gate is approved.
