...
- **Gallery direction:** the prior tall Masonry layout is replaced by a bounded three-row Dome for fine-pointer desktop clients. It uses the same nine local optimized images, a slow 120-second revolution, pauses only for active drag/lightbox, resumes after drag release, opens a tile when pointer-down/up completes without a drag, and keeps a horizontal snap-strip fallback for touch and narrow viewports.
