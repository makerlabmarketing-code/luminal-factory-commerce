-- CART-USD-01: make USD the single cart presentation/persistence currency.
-- Runtime remains default-off. This changes no catalog price, order, payment,
-- inventory, raffle, Auth or ERP data and grants no new browser authority.

set local lock_timeout = '2s';
set local statement_timeout = '15s';

lock table public.carts in access exclusive mode;

alter table public.carts
  drop constraint carts_currency_check;
...
alter table public.carts
  ...
  add constraint carts_currency_check check (currency = 'USD') not valid;

alter table public.carts
  validate constraint carts_currency_check;

create or replace function public.read_verified_customer_cart(
  p_auth_user_id uuid
)
returns jsonb
language plpgsql
...
create or replace function public.remove_verified_customer_cart_line(
  p_auth_user_id uuid,
  p_product_id uuid,
  p_variant_id uuid
)
