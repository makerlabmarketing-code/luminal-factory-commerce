...
import { deleteCustomerAddress, listCustomerAddresses, saveCustomerAddress } from "@/features/account/customer-address-server";
...
export async function handleAddressRequest(request: Request): Promise<Response> {
  ...
  if (!environment.ready || process.env.COMMERCE_SAVED_ADDRESSES_ENABLED !== "true") return failure(404, "unavailable");
  const origin = request.headers.get("origin");
  const site = request.headers.get("sec-fetch-site");
  if (request.method !== "GET" && (!origin || !environment.allowedOrigins.has(origin) || (site !== null && site !== "same-origin") || request.headers.get("x-luminal-address-request") !== "1" || request.headers.get("content-type")?.split(";")[0] !== "application/json")) return failure(403, "invalid_request");
  if (request.method === "GET" && origin && !environment.allowedOrigins.has(origin)) return failure(403, "invalid_request");
  try {
    if (request.method === "GET") return Response.json({ ok: true, addresses: await listCustomerAddresses() }, { headers: responseHeaders });
    ...
    else return failure(409, "not_ready");
