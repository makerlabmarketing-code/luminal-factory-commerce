...
export async function listCustomerAddresses(): Promise<CustomerAddress[]> {
  ...
  const { data, error } = await context.client
    ...
    .select("id,customer_id,label,recipient_name,phone,country_code,administrative_area,locality,address_line1,address_line2,postal_code,is_default,created_at,updated_at")
...
export async function setDefaultCustomerAddress(id: string): Promise<void> {
  ...
  const { data, error } = await context.client
    ...
    .select("id")
    .single();
  if (error || !data) throw new Error("Address could not be updated.");
}
