...
export async function POST(request: Request, { params }: RaffleRouteProps) {
  ...
  if (!parsed.success) return commerceAdminFailure(context.identity.requestId, 400, "REQUEST_INVALID", "Unpublish request không hợp lệ.");

  try {
    const raffle = await unpublishManagedRaffle(context.privilegedClient, raffleId.data, parsed.data, {
      clientId: context.identity.clientId,
      requestFingerprint: context.requestFingerprint,
    });
    await recordCommerceAdminAudit(context, {
      operation: "raffle.unpublish",
      targetType: "raffle",
      targetId: raffle.id,
      outcome: "succeeded",
      httpStatus: 200,
    ...
    return commerceAdminFailure(context.identity.requestId, status, known ? error.code : "REMOTE_UNAVAILABLE", known ? error.message : "Không thể unpublish Raffle.", status === 503);
  }
}
