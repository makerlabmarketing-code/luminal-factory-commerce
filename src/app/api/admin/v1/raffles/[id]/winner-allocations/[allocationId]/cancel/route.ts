import { z } from "zod";
import { raffleWinnerCancelSchema } from "@/features/management/raffle-ops-admin-contract";
import { cancelRaffleWinnerAllocation, RaffleOpsAdminServiceError } from "@/features/management/raffle-ops-admin-service";
...
export async function POST(request: Request, { params }: Props) {
  ...
  let parsed: ReturnType<typeof raffleWinnerCancelSchema.safeParse>;
  try { parsed = raffleWinnerCancelSchema.safeParse(parseCommerceAdminJson(context.rawBodyText)); }
  catch { parsed = { success: false, error: null } as never; }
  if (!parsed.success) return commerceAdminFailure(context.identity.requestId, 400, "REQUEST_INVALID", "Cancellation payload không hợp lệ.");
  try {
    const result = await cancelRaffleWinnerAllocation(context.privilegedClient, raffleId.data, allocation.data, parsed.data, {
      clientId: context.identity.clientId, actorId: context.identity.actorId, requestFingerprint: context.requestFingerprint,
    });
    await recordCommerceAdminAudit(context, { operation: "raffle.winner.cancel", targetType: "raffle_winner_allocation", targetId: allocation.data, outcome: "succeeded", httpStatus: 200 });
    ...
    return commerceAdminFailure(context.identity.requestId, status, known ? error.code : "REMOTE_UNAVAILABLE", known ? error.message : "Không thể hủy winner allocation.", status === 503);
  }
}
