...
import { HomepageHeroAdminServiceError, unpublishHomepageHero } from "@/features/management/homepage-hero-admin-service";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await recordCommerceAdminAudit(context, { operation: "homepage_hero.unpublish", targetId: id.data, outcome: "failed", httpStatus: status, failureCode: code }).catch(() => undefined);
    return commerceAdminFailure(context.identity.requestId, status, code, notFound ? "Homepage Hero không tồn tại." : conflict ? "Homepage Hero operation conflicts with existing state." : "Không thể unpublish Homepage Hero.", status === 503);
  }
}
