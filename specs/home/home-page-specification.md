...
- `HERO-POINTER-01`: eligible fine-pointer desktop clients keep the existing Hero object front-facing and static until the visitor holds and drags. Dragging moves the supported `camera-orbit` with an inverse camera-to-object mapping, so the visible object follows the pointer direction, within a bounded `22°` horizontal and `8°` vertical range while the authored model orientation remains static. Pointer response uses a near-immediate follow rate of `20` with `0.16°/px` horizontal and `0.1°/px` vertical sensitivity; release, cancel, or leaving the Hero interpolates the camera back to center. This avoids mutating the model scene graph on animation frames. Autonomous rotation is disabled at the renderer even when an older remote record still enables it. Existing poster/network capability gates remain intact, and interaction remains disabled for reduced motion and coarse pointers.
- `GALLERY-DOME-01`: the approved nine-image Colorway Studies set may render as a compact three-row curved Dome on fine-pointer desktop clients. It rotates at approximately `3°` per second, pauses only during an active drag or enlarged-image viewing, and resumes shortly after drag release. A click without drag opens the selected image. Touch and narrow viewports use a readable horizontal snap strip; reduced motion disables automatic rotation.
