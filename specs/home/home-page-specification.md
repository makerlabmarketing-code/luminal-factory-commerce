...
- `HERO-POINTER-01`: eligible fine-pointer desktop clients keep the existing Hero object front-facing and static until the visitor holds and drags. Dragging rotates the object within a bounded `22°` horizontal and `8°` vertical range; release, cancel, or leaving the Hero interpolates back to the authored orientation. Autonomous rotation is disabled at the renderer even when an older remote record still enables it. Existing poster/network capability gates remain intact, and interaction remains disabled for reduced motion and coarse pointers.
