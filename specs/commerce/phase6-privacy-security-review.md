...
On 2026-08-26 the separately guarded guest-cart workflow passed its isolated Preview create/delete smoke and returned cart/Auth data to zero rows; the Preview flag was restored to false. Slice B now has a local default-off Account UI with OTP request/verification, explicit Turnstile, exact-origin and strict-input checks, generic failures, cookie refresh, verified-session presentation and local sign-out. Its private keyed-hash limiter was applied once as `20260826105102_add_customer_auth_rate_limits` and passed zero-row postflight. The owner reported SMTP/Turnstile and Preview site-key setup complete, but no OTP staging smoke has run. Customer RLS, address storage, cart attachment, global navigation and Production Auth remain disabled; the prepared exact-head staging runbook requires separate approval before creating the test Auth user.
