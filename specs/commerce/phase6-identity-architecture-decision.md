...
The owner approved and completed the isolated Slice A guest-cart smoke: the guarded GitHub Actions run created and deleted exactly one cart, postflight returned to zero carts/items/Auth users, and runtime was restored to false. PR #43 then delivered the default-off `/account` OTP/Turnstile UI, strict request boundary, cookie session refresh, verified-user presentation, local sign-out and durable keyed-hash limiter to Production as `a7da2b6f9602118342d84b481c8ba30ca7ef4880`. On 2026-08-28 the bounded Production Auth smoke completed after one safe stop to correct an eight-digit OTP setting on the wrong Supabase project. The corrected Commerce project issued a six-digit OTP, verification and refresh succeeded, local sign-out revoked the session, and Auth returned to false on same-source deployment `dpl_5ktST19uiu1wV8nxg9rePRrxkQ2x`. One signed-out Auth user remains; sessions, customers, carts and cart items are zero. Customer RLS, cart attachment, addresses and global Account navigation remain disconnected. The next gate is a separate default-off customer-link and transactional guest-cart merge technical plan.
